...
app.prepare().then(async () => {
  ...
  apiRoutes.post('/api/cotizador', async (ctx)=>{
    console.log("body del cotizador", ctx.body);
    ctx.body = {
        status: "COTIZADOR",
  ...
  router.post("/carrier-service", async (ctx) => {
    ...
    const carrier = await client.post({
      ...
      data: {"carrier_service":{"name":"TCC Cumple","callback_url":"https:\/\/rocketfy-shopify-app.herokuapp.com/api/cotizador","service_discovery":true}},
      type: DataType.JSON,
