...
const getAcessToken  = ()=>{
    return new Promise(async (resolve, reject)=>{
        ...
        const response = await fetch(`${process.env.ROCKETFY_APIHOST}/api/public/connect`, options).catch(e=>reject(e));
        const data = await response.json().catch(e=>reject(e));
...
const refreshToken = ()=>{
    return new Promise(async (resolve, reject)=>{
        ...
        const response = await fetch(`${process.env.ROCKETFY_APIHOST}/api/public/refreshCustomerToken`, options).catch(e=>reject(e));
        const data = await response.json().catch(e=>reject(e));
