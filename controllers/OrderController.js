...
const url =
  process.env.NODE_ENV == "production"
    ? process.env.APIPUBLIC_PRO
    : process.env.APIPUBLIC_DEV;
...
const OrderController = {
  createOrder: (data, auth, client, Graphql) => {
    return new Promise(async (resolve, reject) => {
      ...
      let rs = await axios.post("https://city-predictor.herokuapp.com/cities", {
        query: data.shipping_address.city,
      });
      let city;
      let state;
      ...
      let order = {
        ...
        carrier:
          parseInt(data.total_shipping_price_set.shop_money.amount) == 0
            ? "servientrega"
      ...
      if (order.line_items.length > 0) {
        for (let index = 0; index < order.line_items.length; index++) {
          ...
          order.line_items[index].dimensions.width =
            rs.body.metafields.find((m) => m.key == "ancho").value || 0;
          order.line_items[index].dimensions.height =
            rs.body.metafields.find((m) => m.key == "alto").value || 0;
          order.line_items[index].dimensions.large =
            rs.body.metafields.find((m) => m.key == "largo").value || 0;
      ...
      let o = await axios
        .post(
          `${url}api/public/v2/createOrders`,
  ...
  getShippingRates: (data, auth) => {
    ...
    let weight = (items) => {
      ...
      if (items.length > 0) {
        for (let index = 0; index < data.length; index++) {
          ...
          total =
            total +
            (parseInt(element.quantity) * parseInt(element.grams)) / 1000;
        }
      }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let rates = await axios
        .post(`${url}api/public/v2/calculateShipping`, body, {
          headers: headers,
        })
        .catch((e) => reject(e));

      if (rates && rates.data) {
        rates.data.data.courriers = rates.data.data.courriers
          .filter((x) => x.key != "interrapidisimo")
          .filter((s) => s.key != "rocketfy");
